...
export default function FeedbackManager({ feedback, testimonials }: { feedback: FeedbackRecord[]; testimonials: TestimonialRecord[] }) {
  ...
  return <>
    ...
    {tab === "feedback" ? <section className="feedback-inbox">
      ...
      {testimonials.length ? <div className="testimonial-admin-grid">{testimonials.map((item) => <article key={item.id} className={item.is_featured ? "featured" : ""}>
        <div className="testimonial-admin-person">
          <span>{item.headshot_key ? <Image src={`/api/testimonial-image/${item.id}`} alt={item.image_alt || `Portrait of ${item.person_name}`} width={84} height={84} unoptimized/> : initials(item.person_name)}</span>
          <div><b>{item.person_name}</b><small>{item.designation}{item.institution ? ` · ${item.institution}` : ""}</small></div>
        </div>
        <blockquote title={item.quote}>“{item.quote}”</blockquote>
        <footer>
          <div className="testimonial-admin-badges"><span className={item.is_published ? "live" : "draft"}>{item.is_published ? "Published" : "Draft"}</span>{item.is_featured && <span className="featured">Featured</span>}</div>
          <div className="testimonial-admin-actions"><button type="button" onClick={() => { setEditing(item); setCreating(false); }}>Edit</button><button type="button" className="danger" onClick={() => { if (window.confirm(`Delete ${item.person_name}’s testimonial?`)) run(() => deleteTestimonial(item.id, item.headshot_key ?? "")); }}>Delete</button></div>
        </footer>
      </article>)}</div> : <div className="admin-empty testimonial-empty"><b>No testimonials yet</b><p>Add an honest, consented quote from a principal, HOD or teacher.</p><button onClick={() => setCreating(true)}>Create the first testimonial →</button></div>}
    </section>}
