...
export default async function AdminFlashcardsPage() {
  ...
  const [{ data: subjects, error: subjectError }, { data: cards, error: cardError }] = await Promise.all([
    ...
    client.database.from("content_items").select("id,subject_id,content_type,title,description,body,academic_year,file_url,file_key,sort_order,is_published").eq("content_type", "flashcard").order("sort_order", { ascending: true }),
  ]);
  const error = subjectError ?? cardError;
  return <AdminShell active="/admin/flashcards" email={user.email ?? "Admin"} eyebrow="ACTIVE RECALL" title="Flashcard management">
    ...
    {error ? <div className="admin-notice error">{error.message ?? "Could not load flashcards."}</div> : <FlashcardManager subjects={(subjects ?? []) as SubjectRecord[]} cards={(cards ?? []) as ContentRecord[]}/>} 
  </AdminShell>;
}
