...
export default function FlashcardManager({ subjects, cards, units, topics }: { subjects: SubjectRecord[]; cards: ContentRecord[]; units: FlashcardUnitRecord[]; topics: FlashcardTopicRecord[] }) {
  ...
  function run(action: () => Promise<AdminActionResult>, close = false) {
    startTransition(async () => {
      try {
        ...
        setNotice({ ok: false, message: "The action could not be completed. Please refresh and try again." });
      }
    });
  }
  function unitDeleteMessage(item: FlashcardUnitRecord) {
    const topicCount = topics.filter((entry) => entry.unit_id === item.id).length;
    const cardCount = cards.filter((entry) => entry.flashcard_unit_id === item.id).length;
    return `Delete “${item.title}”?\n\nThis will permanently delete ${topicCount} ${topicCount === 1 ? "topic" : "topics"} and ${cardCount} ${cardCount === 1 ? "flashcard" : "flashcards"} inside this unit. This cannot be undone.`;
  }
  function topicDeleteMessage(item: FlashcardTopicRecord) {
    const cardCount = cards.filter((entry) => entry.flashcard_topic_id === item.id).length;
    return `Delete “${item.title}”?\n\nThis will permanently delete ${cardCount} ${cardCount === 1 ? "flashcard" : "flashcards"} inside this topic. This cannot be undone.`;
  }
  ...
  return <>
    ...
    {manageStructure && <section className="flashcard-structure-manager"><div><header><span>UNITS</span><b>{subject?.name}</b></header>{subjectUnits.length ? subjectUnits.map((item) => <article key={item.id}><div><b>{item.title}</b><small>{topics.filter((entry) => entry.unit_id === item.id).length} topics · {item.is_published ? "Published" : "Draft"}</small></div><button disabled={pending} onClick={() => setStructureEditor({ kind: "unit", record: item })}>Edit</button><button className="danger" disabled={pending} onClick={() => { if (confirm(unitDeleteMessage(item))) run(() => deleteFlashcardUnit(item.id)); }}>Delete</button></article>) : <p>No units created for this subject.</p>}</div><div><header><span>TOPICS</span><b>{unit?.title ?? "Choose a unit"}</b></header>{unitTopics.length ? unitTopics.map((item) => <article key={item.id}><div><b>{item.title}</b><small>{cards.filter((entry) => entry.flashcard_topic_id === item.id).length} cards · {item.is_published ? "Published" : "Draft"}</small></div><button disabled={pending} onClick={() => setStructureEditor({ kind: "topic", record: item })}>Edit</button><button className="danger" disabled={pending} onClick={() => { if (confirm(topicDeleteMessage(item))) run(() => deleteFlashcardTopic(item.id)); }}>Delete</button></article>) : <p>{unit ? "No topics created in this unit." : "Select a unit to manage its topics."}</p>}</div></section>}
        ...
        return <article key={item.id} className={isOpen ? "open" : ""}>
          ...
          {isOpen && <div id={`admin-topic-${item.id}`} className="flashcard-topic-panel">
            ...
            <div className="content-list flashcard-admin-list">{topicCards.length ? topicCards.map((card, index) => <article key={card.id}>
              ...
              <div className="content-item-actions"><button className={`admin-publish-toggle ${card.is_published ? "live" : "draft"}`} disabled={pending} onClick={() => run(() => setContentPublished(card.id, !card.is_published))}><i/>{card.is_published ? "Live" : "Draft"}</button><button disabled={pending} onClick={() => { setEditing(card); setCreating(false); setUnitId(card.flashcard_unit_id ?? ""); setTopicId(item.id); setTopicQuery(item.title); }}>Edit</button><button className="danger" disabled={pending} onClick={() => { if (confirm("Delete this flashcard permanently? This cannot be undone.")) run(() => deleteContentItem(card.id)); }}>Delete</button></div>
